...
| **Eval Harness** | JSONL 任务集，覆盖文本、正则、产物、记忆、上下文和工具错误，适合 CI/回归 | CI / Regression |
...
`dream` 是 Evolva 的本地 Dreaming 循环：它会扫描最近 Trace、最新 Eval 报告和当前 Memory/Skill 覆盖，执行 **Evidence → Hypothesis → Critique → Action**。流程会先收集证据，再生成可证伪假设，随后用确定性的 drift guard 拒绝低置信、重复或缺少证据的建议，最后写入 `evolva/dreams/*.json` 审计报告；加上 `--apply` 后，只把通过质量门的高置信 proposal 写入 Memory / Skill。
...
```json
{
  ...
  "nodes": [
    ...
    {"id": "review", "type": "role", "role": "reviewer", "task": "基于 {{recall}} 和 {{policy}} 评审当前自我进化安全边界"}
  ]
}
```
...
```json
{"id":"policy_trace_001","input":"执行一次带工具调用的安全检查任务","expected_contains":["ok"],"scorers":["no_tool_error"]}
```
...
Evolva 的评测与工程检查可接入 CI，用于守住 Trace / Eval / Self-Evolution 的回归基线。
